...
type CandidateTab = "recommendation" | "strengthen" | "reasoning";
...
function extractDraftTextFromFinalDrafts(
  drafts: WorkspaceFinalDrafts | null | undefined,
  kind: "cv" | "cover",
): string {
  if (!drafts) return "";
  const asDraftRecord = asRecord(drafts.drafts);
  if (kind === "cv") {
    return (
      drafts.finalCv ||
      drafts.cvDraft ||
      asString(asDraftRecord?.finalCv) ||
      asString(asDraftRecord?.cvDraft) ||
      ""
    );
  }
  return (
    drafts.finalCoverLetter ||
    drafts.coverLetterDraft ||
    asString(asDraftRecord?.finalCoverLetter) ||
    asString(asDraftRecord?.coverLetterDraft) ||
    ""
  );
}
...
function getVerdictConfig(rec: string | undefined) {
  switch (rec) {
    ...
    case "apply_with_care":
      return {
        label: "This is a good fit — we just need to align your positioning",
        color: "#92400e",
        bg: "#fef3c7",
        border: "#fcd34d",
    ...
    case "borderline":
      return {
        label: "This role has some gaps, but we can try",
        color: "#9a3412",
        bg: "#ffedd5",
        border: "#fdba74",
        description:
    ...
    case "not_recommended":
      return {
        label: "This role is not for you — but we can still try",
        color: "#7f1d1d",
        bg: "#fee2e2",
        border: "#fca5a5",
        description:
...
type ClarificationQuestion = {
  id: "consolidation" | "policy" | "spreadsheet";
  prompt: string;
};

function getVerdictHeadline(rec: string | undefined, label: string): {
  line1: string;
  line2: string;
} {
  switch (rec) {
    case "apply_confidently":
      return {
        line1: label,
        line2: "You are aligned for this role.",
      };
    case "apply_with_care":
      return {
        line1: "This is a good fit",
        line2: "We just need to align your positioning",
      };
    case "borderline":
      return {
        line1: "This role has some gaps",
        line2: "But we can still position you honestly",
      };
    case "not_recommended":
      return {
        line1: "This role is likely a weak fit",
        line2: "But we can still try with clear positioning",
      };
    default:
      return {
        line1: label,
        line2: "Run the analysis to get a recommendation.",
      };
  }
}

function hasSeniorityMismatchSignal(insights: WorkspaceInsights | null): boolean {
  const haystack = [
    ...extractList(insights?.riskAreas),
    asString(insights?.reasoningSummary) ?? "",
    asString(insights?.advisorMessage) ?? "",
  ]
    .join(" ")
    .toLowerCase();

  return (
    haystack.includes("below your current seniority level") ||
    haystack.includes("underutilize your experience") ||
    haystack.includes("materially below your current scope")
  );
}
...
function buildClarificationQuestions(
  ...
  const text = [...blockers, ...riskAreas, ...missingSignals].join(" ").toLowerCase();
  const out: ClarificationQuestion[] = [];

  if (/(consolidat|group report|group accounting)/.test(text)) {
    out.push({
      id: "consolidation",
      prompt:
        "Have you supported consolidated financial statements or consolidation reporting, even if this is not stated explicitly in your CV?",
    });
  }

  if (/(policy|manual|guideline|sop|procedure)/.test(text)) {
    out.push({
      id: "policy",
      prompt:
        "Have you contributed to accounting policies, manuals, SOPs, or finance guidelines?",
    });
  }

  if (/(excel|spreadsheet|pivot|vlookup|lookup|reconciliation|analysis model)/.test(text)) {
    out.push({
      id: "spreadsheet",
      prompt:
        "Which spreadsheet tasks have you used in practice, for example reporting files, pivot tables, lookups, reconciliations, or analysis models?",
    });
  }
...
function toClarificationLabel(id: string): string {
  const normalized = id.trim().toLowerCase();
  if (normalized === "consolidation") {
    return "Consolidation experience";
  }
  if (normalized === "policy") {
    return "Accounting policies / manuals / SOPs";
  }
  if (normalized === "spreadsheet") {
    return "Spreadsheet / Excel tasks";
  }
  return normalized
    .split(/[_\s-]+/)
    .filter(Boolean)
...
function buildClarificationEvidenceBlock(
  answers: Record<string, string>,
): { text: string; lines: string[] } {
  const lines = Object.entries(answers)
    .map(([id, value]) => ({
      label: toClarificationLabel(id),
      value: value.trim(),
    }))
  ...
  return {
    text: ["Additional candidate clarifications:", ...lines].join("\n"),
    lines,
  };
}

function buildArgumentEvidenceGroups(input: {
  candidateProfile: unknown;
  selectedEvidence: string[];
  positioningArtifact: unknown;
}): Array<{ title: string; items: string[] }> {
  const profile = asRecord(input.candidateProfile);
  const roles = asArray(profile?.roles);
  const roleAchievements = roles.flatMap((role) => {
    const r = asRecord(role);
    return asStringArray(r?.achievements);
  });
  const profileSignals = [
    ...asStringArray(profile?.coreSkills),
    ...asStringArray(profile?.tools),
    ...asStringArray(profile?.standards),
    ...asStringArray(profile?.leadershipSignals),
  ];
  const positioningLines = extractPositioningBriefText(input.positioningArtifact)
    .split("\n")
    .map((line) => line.trim())
    .filter((line) => line.length > 0 && !line.startsWith("Core why fit:") && !line.startsWith("Positioning"));

  const all = Array.from(new Set([...input.selectedEvidence, ...roleAchievements, ...profileSignals, ...positioningLines]));

  const groups = [
    {
      title: "Financial reporting",
      test: (value: string) => /(report|statement|closing|forecast|budget|liquidity|finance)/i.test(value),
    },
    {
      title: "IFRS / standards",
      test: (value: string) => /(ifrs|gaap|standard|policy|compliance|accounting)/i.test(value),
    },
    {
      title: "SAP / systems",
      test: (value: string) => /(sap|erp|oracle|system|tool|automation|digitali)/i.test(value),
    },
    {
      title: "Leadership / coordination",
      test: (value: string) => /(lead|manage|coordina|team|mentor|cross-functional|stakeholder)/i.test(value),
    },
    {
      title: "Auditor / stakeholder interaction",
      test: (value: string) => /(auditor|audit|bank|board|management|stakeholder|external|internal)/i.test(value),
    },
  ];
  ...
  return groups
    .map((group) => ({
      title: group.title,
      items: all.filter((item) => group.test(item)).slice(0, 6),
    }))
...
export default function WorkspaceInsightsPage() {
  ...
  >(state.recommendationClarifications ?? {});
  const [recommendationSavedNotice, setRecommendationSavedNotice] = useState<
    string | null
  >(null);
  const [recommendationProgressMsgIdx, setRecommendationProgressMsgIdx] =
    useState(0);
  ...
  const originalDrafts =
    state.originalFinalDrafts && Object.keys(state.originalFinalDrafts).length
      ? state.originalFinalDrafts
      : state.finalDrafts;
  const originalCvDraft = extractDraftTextFromFinalDrafts(originalDrafts, "cv");
  const originalCoverLetterDraft = extractDraftTextFromFinalDrafts(
    originalDrafts,
    "cover",
  );
  ...
  const baseVerdictHeadline = getVerdictHeadline(
    insights?.applicationRecommendation,
    verdict.label,
  );
  const seniorityMismatchVisible = hasSeniorityMismatchSignal(insights);
  const verdictHeadline = seniorityMismatchVisible
    ? {
        line1: "This role may be below your current level",
        line2: "It could underutilize your experience, even if the skills match",
      }
    : baseVerdictHeadline;
  ...
  const groupedEvidence = buildArgumentEvidenceGroups({
    candidateProfile,
    selectedEvidence,
    positioningArtifact,
  });
  const clarificationAuditRecord = asRecord(bundle?.candidateClarifications);
  const clarificationAuditAnswers = asRecord(
    clarificationAuditRecord?.answers,
  );
  const clarificationAuditSignals = asArray(
    clarificationAuditRecord?.signals,
  )
    .map((item) => asString(asRecord(item)?.label))
    .filter((item): item is string => Boolean(item));
  const clarificationEvidenceUsed = asStringArray(
    asRecord(bundle?.selectedEvidence)?.clarificationEvidenceUsed,
  );
  const clarificationUsageRecord = asRecord(bundle?.clarificationUsage);
  const clarificationProfileUsage = asStringArray(
    clarificationUsageRecord?.profile,
  );
  const clarificationRecommendationUsage = asStringArray(
    clarificationUsageRecord?.recommendation,
  );
  const clarificationCvUsage = asStringArray(clarificationUsageRecord?.cv);
  const clarificationCoverLetterUsage = asStringArray(
    clarificationUsageRecord?.coverLetter,
  );
  const clarificationTruthUsage = asArray(
    clarificationUsageRecord?.truthLayer,
  )
    .map((item) => {
      const record = asRecord(item);
      if (!record) return null;
      ...
      return `${action.toUpperCase()}: ${signal}${reason ? ` (${reason})` : ""}`;
    })
    .filter((item): item is string => Boolean(item));
  const strengthenStrongItems = Array.from(
    new Set([...strongMatches, ...selectedEvidence]),
  ).slice(0, 8);
  const strengthenGapItems = Array.from(
    new Set([...blockers, ...riskAreas, ...missingSignals]),
  ).slice(0, 8);

  const hasInsights = Boolean(insights);
  const clarificationUpdateStatus = state.clarificationUpdateStatus;
  const clarificationUpdateError = state.clarificationUpdateError;
  const clarificationRerunActive = clarificationUpdateStatus === "rerunning";
  const clarificationRerunFailed = clarificationUpdateStatus === "rerun_failed";
  const clarificationRerunSucceeded =
    clarificationUpdateStatus === "rerun_succeeded";
  const recommendationResultMode =
    clarificationRerunSucceeded && !showClarificationEditor;
  const showClarificationInputs =
    !recommendationResultMode;
  const jobTitle =
  ...
  useEffect(() => {
    setRecommendationAnswers(state.recommendationClarifications ?? {});
  }, [state.recommendationClarifications]);

  useEffect(() => {
    if (state.clarificationUpdateStatus === "rerun_succeeded") {
      setShowClarificationEditor(false);
      setRecommendationAnswers({});
    }
  }, [state.clarificationUpdateStatus]);

  useEffect(() => {
    if (state.clarificationUpdateStatus !== "rerunning") return;
    const startedAt = state.clarificationUpdateStartedAt
      ? Date.parse(state.clarificationUpdateStartedAt)
      : NaN;
    ...
    if (elapsedMs > 120000) {
      setClarificationUpdateState({
        status: "rerun_failed",
        error:
          "The previous update appears to have been interrupted. You can retry safely.",
      });
    }
  }, [
    state.clarificationUpdateStatus,
    state.clarificationUpdateStartedAt,
    setClarificationUpdateState,
  ]);
  ...
  async function handleRecommendationUpdateProfile(forceRetry = false) {
    if (clarificationRerunActive && !forceRetry) return;
    // Audit note: this state machine is persisted in workspace state to survive
    // navigation during in-flight reruns and avoid broken mixed Insights UI.
    setClarificationUpdateState({
      status: "saving_clarifications",
      error: null,
      startedAt: new Date().toISOString(),
    });
    setRecommendationClarifications(recommendationAnswers);
    setPipelineProfile((current) => current ?? state.candidateProfile);
    setRecommendationSavedNotice("Your answers were saved.");
    setClarificationUpdateState({
      status: "rerunning",
      error: null,
    });
    setRecommendationProgressMsgIdx(0);

    progressIntervalRef.current = setInterval(() => {
      setRecommendationProgressMsgIdx((prev) =>
        Math.min(prev + 1, PROGRESS_MESSAGES.length - 1),
      );
    }, 3500);

    const result = await runRecommendationPipeline(
      pipelineProfile ?? state.candidateProfile,
    );
    ...
    if (result.ok) {
      ...
      setClarificationUpdateState({
        ...
        error:
          result.message ||
          "Update failed. You can still proceed with your current drafts.",
        startedAt: null,
      });
    }
  }

  async function runRecommendationPipeline(profileOverride: unknown) {
    const profileToSend =
    ...
    if (!profileToSend) {
      return {
        ok: false as const,
        message:
          "No profile available to update yet. Complete the profile step first.",
      };
    }
    ...
    try {
      ...
      const clarificationEvidence = buildClarificationEvidenceBlock(
        recommendationAnswers,
      );
      if (clarificationEvidence.text) {
        payload.candidateClarificationsText = clarificationEvidence.text;
      }
      payload.candidateClarifications = recommendationAnswers;
      if (state.jobText?.trim()) {
        payload.jobDescriptionText = state.jobText.trim();
      }
      if (state.jobUrl?.trim()) {
        payload.jobUrl = state.jobUrl.trim();
      }
      ...
      // Keep rerun application atomic to avoid mixed state between insights/drafts.
      if (!data.insights || !data.finalDrafts) {
        return {
          ok: false as const,
          message:
            "Rerun completed incompletely. Your current drafts were preserved.",
        };
      }

      setInsights(data.insights);
      setFinalDrafts(data.finalDrafts);
      const rerunCandidateProfile = asRecord(data.insights?.bundle)?.candidateProfile;
      if (rerunCandidateProfile && typeof rerunCandidateProfile === "object") {
        setCandidateProfile(rerunCandidateProfile as WorkspaceCandidateProfile);
      }
  ...
  function renderDocumentText(_text: string | undefined | null) {
    return null;
  }

  const currentCvText = null;
  const currentCoverLetterText = null;
              ...
              <div style={{ display: "grid", gap: 12 }}>
                <details open>
                  ...
                  <div style={docPreviewStyle}>
                    {originalCvDraft ? (
                      <div style={sectionContentStyle}>{originalCvDraft}</div>
                    ) : (
                      <div style={emptyCardStyle}>
                        No CV draft available yet.
                      </div>
                    )}
                  </div>
                </details>
                ...
                <details open>
                  ...
                  <div style={docPreviewStyle}>
                    {originalCoverLetterDraft ? (
                      <div style={sectionContentStyle}>
                        {originalCoverLetterDraft}
                      </div>
                    ) : (
                      <div style={emptyCardStyle}>
                        No cover letter draft available yet.
                      </div>
                    )}
                  </div>
                </details>
      ...
      {activeTab === "recommendation" && (
        <div style={{ display: "grid", gap: 16 }}>
          {!hasInsights ? (
            ...
            clarificationRerunActive ? (
              <AppCard className="p-6">
                <div style={emptyCardStyle}>
                  We are still updating your profile from your clarifications.
                  You can stay here, or retry if this screen was reopened after
                  navigation.
                </div>
                <div style={{ ...emptyCardStyle, marginTop: 12 }}>
                  {PROGRESS_MESSAGES[recommendationProgressMsgIdx]}
                </div>
                <div
                  style={{
                    marginTop: 12,
                ...
                >
                  <button
                    type="button"
                    onClick={() => void handleRecommendationUpdateProfile(true)}
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      setFinalDraftPreference("original");
            ...
            <AppCard className="p-6">
              {clarificationRerunFailed && (
                <div style={{ ...errorBannerStyle, marginTop: 0 }}>
                  {clarificationUpdateError ||
                    "Profile update failed. You can retry safely."}
                </div>
              )}
              {clarificationRerunSucceeded && (
                <div style={{ ...emptyCardStyle, marginTop: 0 }}>
                  Your clarifications were used to update this analysis.
                  <div
                    style={{
                      marginTop: 10,
                      display: "flex",
              ...
              >
                ...
                >
                  {verdictHeadline.line2}
                </div>
              </div>

              {showClarificationInputs && clarificationQuestions.length > 0 && (
                <div
                  style={{
                    marginTop: 14,
                    fontSize: 14,
                ...
                >
                  Before we generate your application, I need to clarify a few things.
                </div>
              )}
                ...
                <div style={{ display: "grid", gap: 12, marginTop: 14 }}>
                  {clarificationQuestions.map((question, index) => (
                    <div key={question.id} style={{ display: "grid", gap: 8 }}>
                      ...
                      <textarea
                        value={recommendationAnswers[question.id] ?? ""}
                        onChange={(event) => {
                          const next = {
                            ...recommendationAnswers,
                            [question.id]: event.target.value,
                          };
                          setShowClarificationEditor(true);
                          setRecommendationAnswers(next);
                          setRecommendationClarifications(next);
                          if (
                            state.clarificationUpdateStatus !== "rerunning" &&
                            state.clarificationUpdateStatus !==
              ...
              ) : showClarificationInputs ? (
                <div style={{ ...emptyCardStyle, marginTop: 14 }}>
                  No additional clarification is needed right now.
                </div>
              ) : null}
              ...
              >
                {clarificationQuestions.length > 0 && (
                  <button
                    type="button"
                    onClick={() => void handleRecommendationUpdateProfile()}
                    style={{
                      ...primaryButtonStyle,
                    }}
                  >
              ...
              >
                ...
                >
                  <details open>
                    ...
                    {strengthenStrongItems.length > 0 ? (
                      <ul style={{ ...qualListStyle, marginTop: 10 }}>
                        {strengthenStrongItems.map((item, i) => (
                          <li key={i} style={qualItemStyle}>
                            {item}
                          </li>
                        ))}
                      ...
                      <div style={{ ...emptyCardStyle, marginTop: 10 }}>
                ...
                >
                  <details open>
                    ...
                    {strengthenGapItems.length > 0 ? (
                      <ul style={{ ...qualListStyle, marginTop: 10 }}>
                        {strengthenGapItems.map((item, i) => (
                          <li key={i} style={qualItemStyle}>
                            {item}
                          </li>
                        ))}
                      ...
                      <div style={{ ...emptyCardStyle, marginTop: 10 }}>
                        No major gaps captured right now.
                      </div>
                    )}
                  </details>
                </div>
      ...
      {(internalView === "overview" || internalView === "extraction") && (
        ...
        >
          ...
          <div style={{ display: "grid", gap: 20 }}>
            ...
            <AppCard className="p-6">
              <h2 style={internalTitleStyle}>Clarification audit (internal)</h2>
              <div style={{ display: "grid", gap: 12, marginTop: 14 }}>
                <SectionListBlock
                  title="Answers received"
                  items={
                    clarificationAuditAnswers
                      ? Object.entries(clarificationAuditAnswers)
                          .map(([key, value]) =>
                  ...
                  emptyText="No clarification answers captured."
                />
                <SectionListBlock
                  title="Normalized clarification signals"
                  items={clarificationAuditSignals}
                  emptyText="No normalized clarification signals."
                />
                <SectionListBlock
                  title="Used in selected evidence"
                  items={clarificationEvidenceUsed}
                  emptyText="No clarification signals were used in selected evidence."
                />
                <SectionListBlock
                  title="Used in profile enrichment"
                  items={clarificationProfileUsage}
                  emptyText="No clarification-driven profile enrichment recorded."
                />
                <SectionListBlock
                  title="Used in recommendation"
                  items={clarificationRecommendationUsage}
                  emptyText="No clarification-driven recommendation usage recorded."
                />
                <SectionListBlock
                  title="Used in CV"
                  items={clarificationCvUsage}
                  emptyText="No clarification-driven CV usage recorded."
                />
                <SectionListBlock
                  title="Used in cover letter"
                  items={clarificationCoverLetterUsage}
                  emptyText="No clarification-driven cover letter usage recorded."
                />
                <SectionListBlock
                  title="Truth layer actions"
                  items={clarificationTruthUsage}
                  emptyText="No truth-layer clarification actions recorded."
                />
              </div>
            </AppCard>
