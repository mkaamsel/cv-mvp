...
async function callOpenAI(messages: OpenAIMessage[], temperature = 0.25) {
...
function buildLegacyInputs(body: GenerateCoverLetterRequest): {
  cvText: string;
  jobText: string;
  outputLanguage: OutputLanguage;
  writingLevel: WritingLevel;
  ...
  try {
    ...
    if (!cvText || !jobText) {
      return NextResponse.json(
        {
          error:
            "Cover letter generation requires either (cvText + jobText) or (candidateProfile + structuredJob).",
        },
        { status: 400 }
      );
    }
    ...
    if (cvText.length > CV_LIMIT) {
      return NextResponse.json(
        {
          error:
            "Candidate source content is too long. Please keep it under 14,000 characters.",
        },
        { status: 400 }
      );
    }
    ...
    const systemPrompt = buildGenerateCoverLetterInstructions(locale, writingLevel);
...
Write the final tailored cover letter now.
`.trim();
    ...
    return NextResponse.json({
      ok: true,
      coverLetterDraft: cleanedResult,
